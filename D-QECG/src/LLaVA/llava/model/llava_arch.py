...
from dqecg.visual_selector import select_visual_token_indices
...
def _past_key_values_sequence_length(past_key_values):
    if hasattr(past_key_values, "get_seq_length"):
        return int(past_key_values.get_seq_length())
    return int(past_key_values[-1][-1].shape[-2])
...
class LlavaMetaForCausalLM(ABC):
    ...
    def prepare_d_squared_first_visual_indices(self, image_features):
        ...
        if getattr(self.config, "d_squared_skip_visual_selector", False):
            if isinstance(image_features, list):
                if len(image_features) != 1:
                    raise NotImplementedError("D-QECG supports one image sample at a time.")
        ...
        if keep_count is None:
            raise RuntimeError("D-QECG is enabled, but `d_squared_visual_keep_count` is not set.")

        if isinstance(image_features, list):
            if len(image_features) != 1:
                raise NotImplementedError("D-QECG supports one image sample at a time.")
            visual_tokens = image_features[0]
        else:
            visual_tokens = image_features

        selected = select_visual_token_indices(visual_tokens, keep_num=keep_count)
        if selected.dim() == 2:
            if selected.size(0) != 1:
                raise NotImplementedError("D-QECG supports batch size 1.")
            selected = selected[0]

        model.d_squared_first_visual_indices = selected.detach()
    ...
    def prepare_inputs_labels_for_multimodal(
        ...
        if (
            ...
            cache_length = _past_key_values_sequence_length(past_key_values)
        ...
        if vision_tower is None or images is None or input_ids.shape[1] == 1:
            # During cached generation, prepare_inputs_for_generation no longer
            # carries the image tensor after the multimodal prefill. The cache
            # nevertheless contains the expanded visual-token sequence, so the
            # text-only attention mask must be extended to the cache length.
            if past_key_values is not None and input_ids.shape[1] == 1:
                target_shape = _past_key_values_sequence_length(past_key_values) + 1
                if attention_mask is None:
                    attention_mask = torch.ones(
                        (input_ids.shape[0], target_shape),
                        dtype=torch.bool,
                        device=input_ids.device,
                    )
                elif attention_mask.shape[1] < target_shape:
        ...
        self.prepare_d_squared_first_visual_indices(image_features)
        ...
        for batch_idx, cur_input_ids in enumerate(input_ids):
            ...
            if getattr(self.config, "use_d_squared", False) and num_images != 1:
                raise NotImplementedError("D-QECG supports exactly one image token per sample.")
            if num_images == 0:
