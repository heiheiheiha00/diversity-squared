            ...
            if self.gradient_checkpointing and self.training:
                ...
                if USE_D_SQUARED:
                    if AGG_LAYER is None:
                        raise RuntimeError("D-QECG is enabled, but `d_squared_agg_layer` is not set.")
                    assert AGG_LAYER > 0 , "D-QECG pruning layer should be larger than 0."
                    if SYS_LENGTH is None or IMAGE_TOKEN_LENGTH is None:
                        raise RuntimeError(
                            "D-QECG is enabled, but `d_squared_sys_length` or "
                    ...
                    if (not SKIP_VISUAL_SELECTOR and VISUAL_KEEP_COUNT is None) or LLM_KEEP_COUNT is None:
                        raise RuntimeError(
                            "D-QECG is enabled, but `d_squared_visual_keep_count` or "
                            "`d_squared_llm_keep_count` is not set."
                        )
                    if D_SQUARED_INPLACE and use_cache and not D_SQUARED_STATIC_KV_CACHE:
                        raise RuntimeError(
                            "D-QECG token-drop with KV cache requires `d_squared_static_kv_cache=True`."
                        )



                # D-QECG token drop implementation.
                ...
                # D-QECG attention-mask implementation.
                elif USE_D_SQUARED:
                    if idx<AGG_LAYER:
                        ...
                        if idx!=0:
                            ...
                            raise RuntimeError("D-QECG second-stage selection requires `d_squared_agg_layer > 0`.")
