...
DATA_ROOT = os.environ.get("DATA_ROOT", "/home/majie/majie_data")
dataset = load_from_disk(os.path.join(DATA_ROOT, "aokvqa", "validation"))
...
if __name__=="__main__":
    ...
    parser.add_argument(
        '--model-path',
        type=str,
        default=os.path.join(DATA_ROOT, "base_model", "llava-v1.5-13b"),
    )
    parser.add_argument(
        '--use-dqecg',
        '--use-d-squared',
        ...
        default=False,
        action='store_true',
        help='whether to use D-QECG',
    )
    parser.add_argument('--d-squared-inplace', default=False, action='store_true', help='whether to use token-drop mode')
    parser.add_argument('--d-squared-sys-length', type=int, required=False, help='the length before visual tokens')
    parser.add_argument('--d-squared-image-token-length', type=int, required=False, help='the length of visual tokens')
    parser.add_argument('--d-squared-visual-keep-count', type=int, required=False, help='first-stage visual tokens to keep')
    parser.add_argument('--d-squared-llm-keep-count', type=int, required=False, help='second-stage visual tokens to keep')
    parser.add_argument('--d-squared-agg-layer', type=int, required=False, help='the pruning layer')
    # output path
    ...
    # set model D-QECG config
    if pargs.use_d_squared == True:
        model.config.use_d_squared = True
        model.config.d_squared_inplace = pargs.d_squared_inplace
        model.config.d_squared_sys_length = pargs.d_squared_sys_length
        model.config.d_squared_image_token_length = pargs.d_squared_image_token_length
        model.config.d_squared_visual_keep_count = pargs.d_squared_visual_keep_count
        model.config.d_squared_llm_keep_count = pargs.d_squared_llm_keep_count
        model.config.d_squared_agg_layer = pargs.d_squared_agg_layer
    else:
        model.config.use_d_squared = False
    ...
    def inference(prompts,images):
        ...
        for prompt,image in tqdm(zip(prompts,images),total=len(prompts)):
            ...
            with torch.inference_mode():
                output_ids, sample_latency = measure_call(
            ...
            print(f"{output} | sample_latency={sample_latency * 1000.0:.3f} ms")
        

        return outputs, sample_latencies
    

    # %%
    # inference and compute cider scores
    oakvqa_val_inference_outputs, sample_latencies = inference(valid_prompt,valid_images)
    latency_metrics = latency_report(sample_latencies)
    print(f"mean sample latency: {latency_metrics['mean_seconds'] * 1000.0:.3f} ms")
    ...
    with open(output_path,"w") as f:
        ...
        json.dump(
            {
                "acc": str(acc),
                "output": oakvqa_val_inference_outputs,
                "labels": valid_anwser_options,
