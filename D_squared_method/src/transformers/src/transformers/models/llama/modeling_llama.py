...
from .d_squared_pruning import build_d_squared_keep_indices, build_d_squared_visual_token_indices
from D_squared_method.llm_selector import select_llm_secondary_visual_token_indices
...
class LlamaModel(LlamaPreTrainedModel):
    ...
    def __init__(self, config: LlamaConfig):
        ...
        self.use_d_squared = config.use_d_squared
        self.d_squared_sys_length = config.d_squared_sys_length
        self.d_squared_image_token_length = config.d_squared_image_token_length
        self.d_squared_visual_keep_count = config.d_squared_visual_keep_count
        self.d_squared_llm_keep_count = config.d_squared_llm_keep_count
        self.d_squared_agg_layer = config.d_squared_agg_layer
        self.d_squared_inplace = config.d_squared_inplace
        self.d_squared_skip_visual_selector = getattr(config, "d_squared_skip_visual_selector", False)
        self.d_squared_static_kv_cache = getattr(config, "d_squared_static_kv_cache", False)
    ...
    def reset_d_squared(self):
        self.use_d_squared = self.config.use_d_squared
        self.d_squared_sys_length = self.config.d_squared_sys_length
        self.d_squared_image_token_length = self.config.d_squared_image_token_length
        self.d_squared_visual_keep_count = self.config.d_squared_visual_keep_count
        self.d_squared_llm_keep_count = self.config.d_squared_llm_keep_count
        self.d_squared_agg_layer = self.config.d_squared_agg_layer
        self.d_squared_inplace = self.config.d_squared_inplace
        self.d_squared_skip_visual_selector = getattr(self.config, "d_squared_skip_visual_selector", False)
        self.d_squared_static_kv_cache = getattr(self.config, "d_squared_static_kv_cache", False)
    ...
    def _d_squared_debug_to_cpu(debug):
        ...
        if isinstance(debug, dict):
            return {key: LlamaModel._d_squared_debug_to_cpu(value) for key, value in debug.items()}
        if isinstance(debug, (list, tuple)):
            return type(debug)(LlamaModel._d_squared_debug_to_cpu(value) for value in debug)
        return debug
    ...
    def forward(
        ...
        device = input_ids.device if input_ids is not None else inputs_embeds.device
        d_squared_prefill = bool(self.use_d_squared and past_key_values is None and seq_length > 1)
        d_squared_attention_layer = (
            int(self.d_squared_agg_layer) - 1
            if d_squared_prefill and self.d_squared_agg_layer is not None
            else None
        )
        ...
        for idx, decoder_layer in enumerate(self.layers):
            ...
            layer_output_attentions = bool(
                output_attentions
                or (d_squared_requires_selector_attention and idx == d_squared_attention_layer)
            )
            if self.gradient_checkpointing and self.training:
                ...
                USE_D_SQUARED = self.use_d_squared
                SYS_LENGTH= self.d_squared_sys_length
                IMAGE_TOKEN_LENGTH = self.d_squared_image_token_length
                VISUAL_KEEP_COUNT = self.d_squared_visual_keep_count
                LLM_KEEP_COUNT = self.d_squared_llm_keep_count
                AGG_LAYER = self.d_squared_agg_layer
                D_SQUARED_INPLACE = self.d_squared_inplace
                SKIP_VISUAL_SELECTOR = getattr(self, "d_squared_skip_visual_selector", False)
                D_SQUARED_STATIC_KV_CACHE = getattr(self, "d_squared_static_kv_cache", False)

                if USE_D_SQUARED:
                    if AGG_LAYER is None:
                        raise RuntimeError("D-squared is enabled, but `d_squared_agg_layer` is not set.")
                    assert AGG_LAYER > 0 , "D-squared pruning layer should be larger than 0."
                    if SYS_LENGTH is None or IMAGE_TOKEN_LENGTH is None:
                        raise RuntimeError(
                            "D-squared is enabled, but `d_squared_sys_length` or "
                            "`d_squared_image_token_length` is not set."
                        )
                    if (not SKIP_VISUAL_SELECTOR and VISUAL_KEEP_COUNT is None) or LLM_KEEP_COUNT is None:
                        raise RuntimeError(
                            "D-squared is enabled, but `d_squared_visual_keep_count` or "
                            "`d_squared_llm_keep_count` is not set."
                        )
                    if D_SQUARED_INPLACE and use_cache and not D_SQUARED_STATIC_KV_CACHE:
                        raise RuntimeError(
                            "D-squared token-drop with KV cache requires `d_squared_static_kv_cache=True`."
                        )



                # D-squared token drop implementation.
                if USE_D_SQUARED and D_SQUARED_INPLACE and d_squared_prefill:
                    #print("using inplace")
                    ...
                    if idx<AGG_LAYER:
                        ...
                        selector_attention = previous_layer_attention
                        ...
                        if getattr(self.config, "d_squared_dump_selection_debug", False):
                            second_visual_indices, selection_debug = select_llm_secondary_visual_token_indices(
                                attention=selector_attention,
                                hidden_states=hidden_states,
                                first_selected_indices=first_visual_indices,
                                keep_num=LLM_KEEP_COUNT,
                                image_token_start_index=SYS_LENGTH,
                            ...
                            selection_debug["visual_token_start"] = int(SYS_LENGTH)
                            selection_debug["visual_token_end"] = int(SYS_LENGTH + IMAGE_TOKEN_LENGTH)
                            selection_debug["target_layer"] = int(idx - 1)
                            self.d_squared_last_selection_debug = self._d_squared_debug_to_cpu(selection_debug)
                        else:
                            second_visual_indices = select_llm_secondary_visual_token_indices(
                                attention=selector_attention,
                                hidden_states=hidden_states,
                                first_selected_indices=first_visual_indices,
                                keep_num=LLM_KEEP_COUNT,
                        ...
                        keep_indexs = build_d_squared_keep_indices(
                        ...
                        self.d_squared_final_visual_indices = (
                            keep_indexs[(keep_indexs >= SYS_LENGTH) & (keep_indexs < SYS_LENGTH + IMAGE_TOKEN_LENGTH)]
                            - SYS_LENGTH
                        ).detach()
                        # update seq length
                        ...
                        if use_cache and D_SQUARED_STATIC_KV_CACHE:
                            next_decoder_cache = self._d_squared_trim_cache(next_decoder_cache, keep_indexs)
                            self.d_squared_cache_active = True
                            self.d_squared_cached_sequence_length = int(new_seq_length)
                            self.d_squared_next_position_id = int(seq_length_with_past)
                ...
                elif USE_D_SQUARED and D_SQUARED_INPLACE:
                    new_attention_mask = attention_mask

                # D-squared attention-mask implementation.
                elif USE_D_SQUARED:
                    if idx<AGG_LAYER:
                        ...
                        if idx!=0:
                            ...
                            self.d_squared_final_visual_indices = (keep_visual_indices - SYS_LENGTH).detach()
                            ...
                            gen_attention_mask[:,keep_visual_indices] = True
                            ...
                            raise RuntimeError("D-squared second-stage selection requires `d_squared_agg_layer > 0`.")
