"""D-squared single-layer visual token pruning utilities."""
...
def _require_indices(indices: torch.Tensor, name: str) -> torch.Tensor:
    if indices is None:
        raise RuntimeError(f"D-squared is enabled, but `{name}` is missing.")
...
def build_d_squared_keep_indices(
    ...
    if visual_start < 0 or visual_end > seq_length:
        raise ValueError(
            "D-squared visual token range is outside the current sequence: "
