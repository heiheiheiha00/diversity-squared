...
class LlavaMetaForCausalLM(ABC):
    ...
    def prepare_d_squared_first_visual_indices(self, image_features):
        ...
        if keep_count is None:
            raise RuntimeError("D-squared is enabled, but `d_squared_visual_keep_count` is not set.")
        ...
        if selected.dim() == 2:
            if selected.size(0) != 1:
                raise NotImplementedError("D-squared v1 supports batch size 1.")
            selected = selected[0]
    ...
    def prepare_inputs_labels_for_multimodal(
        ...
        for batch_idx, cur_input_ids in enumerate(input_ids):
            ...
            if getattr(self.config, "use_d_squared", False) and num_images != 1:
                raise NotImplementedError("D-squared v1 supports exactly one image token per sample.")
            if num_images == 0:
